...
for i, ((var, aval), uval, eval) in enumerate(zip(adva.items(), upda.values(), eval.values())):
	...
	if i == 0:
		...
		dashes = (None, None)
	ax.plot(aval, label="adva" if i == 2 else None, color="tab:blue", alpha=alpha, dashes=dashes)
	ax.plot(eval, label="eval" if i == 2 else None, color="tab:orange", alpha=alpha, dashes=dashes)
	ax.plot(uval, label="upda" if i == 2 else None, color="tab:green", alpha=alpha, dashes=dashes)
...
ax.set_ylim(ymin=1e-2, ymax=2e3)
...
ax.legend(handles + [solid_line, dashed_line, dotted_line], labels + ["phercomp", "gpupher", "manyant"], loc="upper left", ncols=2)
