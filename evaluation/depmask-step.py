...
profile_name = r"depmask-samplemask-step"

problem_range = range(150, 350 + 1, 10)

variants = [ "phercomp", "binsearch", "depmask" ]
problems = [f"ESC63s{k}.sop" for k in problem_range]

data = load_data([
    f"evaluation/{profile_name}.profile.csv",
    f"evaluation/binsearch.step.shower.profile.csv",
    f"evaluation/phercomp.step.shower.profile.csv",
])
