...
class ParAnt4Optimizer: public CLColonyOptimizer {
protected:
	cl::Program program;
	cl::KernelFunctor<
...
public:
	static constexpr const char* static_name = "parant4";
	static constexpr const char* static_params = "";

	ParAnt4Optimizer(Problem problem, AntParams params)
	...
	void prepare() override {
		setupCL(true);
		program = loadProgram("./src/variants/parant4.cl");
		...
		ant_sample_d = cl::Buffer(context, CL_MEM_READ_WRITE, sizeof(double) * problem.size() * work_size);
		...
		queue.enqueueWriteBuffer(
			...
			sizeof(double) * zero_doubles2.size(),
			zero_doubles2.data());
		/*queue.enqueueWriteBuffer(
